...
#include "robust_least_squares/huber_loss_function.h"
#include "math.h"
...
double HuberLossFunction::evaluateFunction(const Eigen::VectorXd &errors) const
{
    ...
    {
        if (std::abs(errors(i)) < x_transition_) result += errors(i) * errors(i);
        else result += x_transition_ * (2 * std::abs(errors(i)) - x_transition_);
    }
    return result;
}
...
Eigen::VectorXd HuberLossFunction::evaluateGradient(const Eigen::VectorXd &errors) const
{
    ...
    for (uint i = 0; i < errors.size(); ++i)
    {
        if (std::abs(errors(i)) < x_transition_) result(i) = 2 * errors(i);
        else result(i) = 2 * sign(errors(i)) * x_transition_;
    }
    return result;
}
...
Eigen::MatrixXd HuberLossFunction::evaluateHessian(const Eigen::VectorXd &errors) const
{
    ...
    for (uint i = 0; i < errors.size(); ++i)
    {
        ...
        if (std::abs(errors(i)) < x_transition_) gradient(i) = 2.0;
